...
public class Indexer implements IExecEvents {
	...
	public synchronized void getAstOccVarFromInput(final String input, final String name, final int startOffset, final int endOffset, final ICallBack callBack) {
	...
	/*public synchronized void getAstOccVarFromFile(final String name, final int startOffset, final int endOffset, final String path, final ICallBack callBack) {
		Job job = new Job("Ast Occurrences Variable") {
			@Override
			protected IStatus run(IProgressMonitor arg0) {
				waitReady(); //
