...
public class OcamlASTView extends ViewPart implements ISelectionListener,
		ISelectionChangedListener, ICursorPositionListener {

	private TreeViewer treeViewer;
	private OcamlEditor activeEditor;
		...
		partListener = new IPartListener() {
			public void partActivated(IWorkbenchPart part) {
				...
				if (part instanceof OcamlEditor) {
					...
					// get the text of the document currently opened in the
					// editor
					...
					editor.addCursorPositionListener(OcamlASTView.this);
				}
			}
			...
			public void partClosed(IWorkbenchPart part) {
				if (part instanceof OcamlEditor) {
					OcamlEditor editor = (OcamlEditor) part;
					editor.getTextViewer().getSelectionProvider().removeSelectionChangedListener(
							OcamlASTView.this);
				}
	...
	private void handleSelectedNodeChanged(ASTNode astNode) {
		...
		if (node instanceof Element) {
			...
			if (loc != null) {
				ISelection curSelection = this.activeEditor.getSelectionProvider().getSelection();
				if (curSelection instanceof ITextSelection) {
					...
					try {
						IDocument document = this.activeEditor.getDocumentProvider().getDocument(
								this.activeEditor.getEditorInput());
						int offset = translateLocToDocumentOffset(document, loc.startOffset);

						this.ignoreSelectionEvent = true;
						this.activeEditor.selectAndReveal(offset, length);
					} finally {
	...
	public void cursorPositionChanged(ITextEditor textEditor, Point selectedRange) {
		...
		if (textEditor == activeEditor) {
			...
			final IDocument document = this.activeEditor.getDocumentProvider().getDocument(
					this.activeEditor.getEditorInput());
	...
	/** Reverse operation from {@link #translateLocToDocumentOffset(IDocument, int)} */
